...
Embdr.prototype.process = function(item, options) {
    ...
    var hasAnyCallbacks = (options.start || options.error || options.complete || (options.thumbnails && options.thumbnails.complete));
    ...
    creator(item, createOptions, function(createError, createdResource) {
        ...
        // If no callbacks were provided, we can stop here
        if (!hasAnyCallbacks) {
            return;
        }
