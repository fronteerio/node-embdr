 ...
 * @param  {Object}                 options.imagePreviews               The image previews
 * @param  {string[]}               options.imagePreviews.sizes         The sizes for the image previews. This should be a comma-separated list of sizes of the format {width}x{height}. When one dimension is specified, the image will be scaled appropriately For example, ['1200x', 'x600']
 * @param  {Function}               options.imagePreviews.complete      Called when all the image previews have been processed. If this is a string, it's assumed it's a callback URL where the data should be posted to
...
Embdr.prototype.process = function(item, options) {
    ...
    var hasPollingCallbacks = (options.complete || (options.thumbnails && options.thumbnails.complete));

    // Default the options
    options.start = options.start || function() {};
    options.error = options.error || function() {};
    options.complete = options.complete || function() {};
    options.imagePreviews = options.imagePreviews || {};
    options.imagePreviews.complete = options.imagePreviews.complete || function() {};
    ...
    var callbacks = {
        ...
        'imagePreviews': _.once(options.imagePreviews.complete),
    ...
    var createOptions = {
        ...
        'imagePreviewSizes': options.imagePreviews.sizes
    };
