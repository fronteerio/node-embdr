...
module.exports = Processr;

// Default the host settings where the Processr API can be reached
Processr.DEFAULT_HOST = 'api.processr.com';
Processr.DEFAULT_PORT = '443';
Processr.DEFAULT_PROTOCOL = 'https';
Processr.DEFAULT_STRICT_SSL = true;
Processr.DEFAULT_BASE_PATH = '/api';
...
Processr.DEFAULT_POLLING_INITIAL_TIMEOUT = 2000;
...
Processr.DEFAULT_POLLING_BACKOFF_DENOMINATOR = 4;

/**
 * Create a new Processr instance
 *
 * @constructor
 * @param {string}      key     The API key that allows for uploading to the Processr REST API
 */
function Processr(key) {
    if (!(this instanceof Processr)) {
        return new Processr(key);
    }

    this._api = {
        'auth': null,
        'host': Processr.DEFAULT_HOST,
        'port': Processr.DEFAULT_PORT,
        'basePath': Processr.DEFAULT_BASE_PATH,
        'protocol': Processr.DEFAULT_PROTOCOL
    };
 ...
 * @param  {Function}               options.start                       Called when the item has been created. Its only argument is the resource that was created by the REST API
 * @param  {Function}               options.error                       Called when the REST API could not be reached. Its only argument is the error object explaining what went wrong
 * @param  {Function}               options.complete                    Called when the item has been fully processed and all previews have been generated
 ...
 * @param  {Function}               options.thumbnails.complete         Called when all the thumbnails have been processed
 */
Processr.prototype.process = function(item, options) {
    var self = this;
    ...
    creator(item, createOptions, function(createError, createdResource) {
        ...
        var poll = function() {
            ...
            self.resources.get(createdResource.id, function(err, resource) {
                ...
                var processors = _.chain(resource)
                    .values()
                    // Get the processors
                    .filter(function(val) {
                        return _.has(val, 'status');
                    })
                    .value();

                // Check thumbnails
                var thumbnailProcessors = _.filter(processors, function(processor) {
                    return (processor.processor.indexOf('thumbnail-') === 0);
                });
                var completedThumbnailProcessors = _.filter(thumbnailProcessors, {'status': 'done'});
                if (completedThumbnailProcessors.length === thumbnailProcessors.length) {
                    var thumbnailData = {};
                    _.each(completedThumbnailProcessors, function(processor) {
                        // Cut off the `thumbnail-` portion
                        var dimensions = processor.processor.substring(10);
                        thumbnailData[dimensions] = {
                            'dimensions': dimensions,
                            'url': processor.file.url
                        };
                    });
                    callbacks.thumbnails(thumbnailData);
                }

                var pendingProcessors = _.filter(processors, {'status': 'pending'});

                // If there are no pending processors left, we're done and can return to the caller
                if (_.isEmpty(pendingProcessors)) {
 ...
 */
Processr.prototype.setHost = function(host, port, protocol) {
 ...
 * @param {number}      port            The port on which the REST API can be reached
 */
Processr.prototype.setPort = function(port) {
 ...
 * @param {string}      protocol        The protocol on which the REST API can be reached
 */
Processr.prototype.setProtocol = function(protocol) {
 ...
 */
Processr.prototype.setBasePath = function(basePath) {
 ...
 */
Processr.prototype.setStrictSSL = function(strictSSL) {
    this._setApiField('strictSSL', strictSSL);
};

/**
 * Set the API key that allows for uploading to the Processr REST API
 *
 * @param {string}      key             The API key that allows for uploading to the Processr REST API
 */
Processr.prototype.setApiKey = function(key) {
    if (key) {
 ...
 * @api private
 */
Processr.prototype._setApiField = function(key, value) {
    this._api[key] = value;
};

/**
 * Expose the REST APIs on the `Processr` instance
 *
 * @api private
 */
Processr.prototype._exposeApis = function() {
    var self = this;
    ...
    // Expose each API on the processr instance
 ...
 * @api private
 */
Processr.prototype._request = function(method, path, data, callback) {
